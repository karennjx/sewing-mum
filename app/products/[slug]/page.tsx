...
export default async function ProductPage(
  ...
  // Listing prints is what opts a product into the interactive gallery, where
  // the thumbnails and the print tags both drive one main photo. Those two sit
  // in opposite columns, so they need the client provider wrapped around both;
  ...
  const layout = (
    <div className="mt-6 grid gap-10 lg:grid-cols-2 lg:gap-14">
      {isShowcase ? (
        <ShowcaseGallery images={product.images} />
      ) : (
      ...
      <div>
        ...
        {isSeasonal(product) && seasonWindow ? (
          <p className="mt-4 rounded-card border border-spool/30 bg-spool/10 px-4 py-3 text-sm leading-relaxed text-ink">
            ...
            season and you will be first on the list when the next batch starts.
          </p>
        ) : null}
        ...
        {/* Sits with the button rather than up under the price: the choice and
        ...
        <div className="mt-8">
          ...
          <p className="mt-3 text-xs text-muted">
            ...
            to explain which one you meant.
          </p>
        </div>
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-10 lg:py-14">
      ...
      {isShowcase ? (
        <ProductShowcase images={product.images}>{layout}</ProductShowcase>
      ) : (
        layout
      )}
